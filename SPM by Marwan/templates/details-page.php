...
$status_html = $meta['is_active']
    ? '<span class="spm-status-badge spm-status-badge--active">' . esc_html__('Active', 'smart-performance-monitor') . '</span>'
    : '<span class="spm-status-badge spm-status-badge--inactive">' . esc_html__('Inactive', 'smart-performance-monitor') . '</span>';

// Trust Badge Logic
$trust_status = 'unknown';
$trust_label = __('Unknown', 'smart-performance-monitor');
$trust_class = 'neutral';

if ($trust['verification'] === 'verified' && $security['risk_level'] === 'clean') {
    $trust_status = 'verified';
    $trust_label = __('Verified', 'smart-performance-monitor');
    $trust_class = 'success';
} elseif ($trust['verification'] === 'needs-review' || $security['risk_level'] === 'high' || $security['risk_level'] === 'medium') {
    $trust_status = 'suspicious';
    $trust_label = __('Suspicious', 'smart-performance-monitor');
    $trust_class = 'danger';
...
<div class="spm spm-details-page" id="spm-details-root">
    ...
    <?php if ($isolation_target): ?>
        <div class="spm-isolation-banner">
            ...
            <div class="spm-isolation-banner__text">
                ...
                <?php printf(esc_html__('"%s" is currently deactivated for performance testing.', 'smart-performance-monitor'), esc_html($isolation_target ?? '')); ?>
            </div>
        ...
        <div class="spm-header__inner">
            <div class="spm-header__left">
                <a href="<?php echo esc_url(admin_url('admin.php?page=spm-dashboard')); ?>" class="spm-back-link"
                    title="<?php esc_attr_e('Back to Dashboard', 'smart-performance-monitor'); ?>">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M19 12H5" />
            ...
            <div class="spm-header__right">
                <div class="spm-header__actions">
                    <button class="spm-hdr-btn" data-spm-basename="<?php echo esc_attr($meta['basename'] ?? ''); ?>"
                        ...
                        <?php esc_html_e('Refresh Now', 'smart-performance-monitor'); ?>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="spm-details-tabs">
        <button class="spm-details-tab spm-details-tab--active" data-tab-id="performance">
            ...
            <?php esc_html_e('Performance', 'smart-performance-monitor'); ?>
        </button>
        <button class="spm-details-tab" data-tab-id="security">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
            </svg>
            <?php esc_html_e('Security & Trust', 'smart-performance-monitor'); ?>
        </button>
        <button class="spm-details-tab" data-tab-id="history">
            ...
            <?php esc_html_e('History', 'smart-performance-monitor'); ?>
        </button>
        <button class="spm-details-tab" data-tab-id="logs">
            ...
            <?php esc_html_e('Logs', 'smart-performance-monitor'); ?>
        </button>
    </div>
    ...
    <div class="spm-body-grid">
                    ...
                    <div class="spm-panel__body spm-detail-errors">
                        <?php if (empty($logs)): ?>
                            <p class="spm-detail-empty">
                                <?php esc_html_e('No logs available for this plugin.', 'smart-performance-monitor'); ?>
                            </p>
                        <?php else: ?>
        ...
        <div class="spm-right-col">
            ...
            <div class="spm-panel">
                <div class="spm-panel__header">
                    <h2 class="spm-panel__title"><?php esc_html_e('Plugin Metadata', 'smart-performance-monitor'); ?></h2>
                </div>
                <div class="spm-panel__body">
                    <div class="spm-sidebar-info">
                        <div class="spm-sidebar-row">
                            <span
                                class="spm-sidebar-label"><?php esc_html_e('Author', 'smart-performance-monitor'); ?></span>
                            <span class="spm-sidebar-value"><?php echo esc_html($meta['author'] ?? ''); ?></span>
                        </div>
                        <div class="spm-sidebar-row">
                            <span
                                class="spm-sidebar-label"><?php esc_html_e('Plugin Path', 'smart-performance-monitor'); ?></span>
                            <span
                                class="spm-sidebar-value spm-sidebar-value--code"><code><?php echo esc_html($meta['basename'] ?? ''); ?></code></span>
                        </div>
                        <div class="spm-sidebar-row">
                            <span
                                class="spm-sidebar-label"><?php esc_html_e('Text Domain', 'smart-performance-monitor'); ?></span>
                            <span
                                class="spm-sidebar-value"><?php echo esc_html(!empty($meta['text_domain']) ? $meta['text_domain'] : 'N/A'); ?></span>
                        </div>
                        <div class="spm-sidebar-row">
                            <span
                                class="spm-sidebar-label"><?php esc_html_e('Requires WP', 'smart-performance-monitor'); ?></span>
                            <span
                                class="spm-sidebar-value"><?php echo esc_html(!empty($meta['requires_wp']) ? $meta['requires_wp'] : 'Not specified'); ?></span>
                        </div>
                        <div class="spm-sidebar-row">
                            <span
                                class="spm-sidebar-label"><?php esc_html_e('Requires PHP', 'smart-performance-monitor'); ?></span>
                            <span
            ...
            <div class="spm-panel">
                <div class="spm-panel__header">
                    <h2 class="spm-panel__title"><?php esc_html_e('Full File Path', 'smart-performance-monitor'); ?></h2>
                </div>
                <div class="spm-panel__body">
                    <code
            ...
            <div class="spm-panel">
                <div class="spm-panel__header">
                    <h2 class="spm-panel__title"><?php esc_html_e('Management', 'smart-performance-monitor'); ?></h2>
                </div>
                <div class="spm-panel__body">
                    <div class="spm-details-actions">
                        <?php if ($meta['is_active']): ?>
                            <button class="spm-btn spm-btn--danger spm-btn--full"
                                ...
                                <?php esc_html_e('Deactivate Plugin', 'smart-performance-monitor'); ?>
                            </button>
                            <?php if (!$isolation_target): ?>
                                <button class="spm-btn spm-btn--outline spm-btn--full"
                                    data-spm-basename="<?php echo esc_attr($meta['basename'] ?? ''); ?>"
                                    data-spm-action="isolate"
                                    title="<?php esc_attr_e('Temporarily disable to test site performance without this plugin.', 'smart-performance-monitor'); ?>">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                        stroke-width="2">
                                        <path d="M12 2v20M2 12h20" />
                                    </svg>
                                    <?php esc_html_e('Isolate for Test', 'smart-performance-monitor'); ?>
                                </button>
                            <?php endif; ?>
                        <?php else: ?>
                            <button class="spm-btn spm-btn--success spm-btn--full"
                                ...
                                <?php esc_html_e('Activate Plugin', 'smart-performance-monitor'); ?>
                            </button>
                        <?php endif; ?>
                        <button class="spm-btn spm-btn--outline spm-btn--full" onclick="window.location.reload();">
                            ...
                            <?php esc_html_e('Refresh Page', 'smart-performance-monitor'); ?>
                        </button>
                    </div>
                </div>
            </div>
