 ...
 *
 * All data comes from the centralised SPMBYMA_Query_Service snapshot,
...
class SPMBYMA_Dashboard {

    private SPMBYMA_Data_Service $data_service;

    public function __construct( SPMBYMA_Data_Service $data_service ) {
    ...
    public function render(): void {
        ...
        $license_report = SPMBYMA_License_Detector::get_cached_scan();
        if ( ! $license_report ) {
        ...
        <div class="spm" id="spm-dashboard">
            ...
            <div class="spm-body-grid">
                ...
                <div class="spm-right-col">
                    ...
                    <!-- Support Development -->
                    <?php include SPMBYMA_PLUGIN_DIR . 'templates/support-card.php'; ?>
    ...
    private function render_score_gauge( float $score ): void {
        $grade = SPMBYMA_Analyzer::score_to_grade( (int) round( $score ) );
    ...
    private function render_period_filter( int $active_period ): void {
        ...
        <div class="spm-period-filter">
            <?php foreach ( SPMBYMA_Data_Service::PERIOD_LABELS as $days => $label ) :
                $is_active = ( $days === $active_period );
                $url       = add_query_arg( 'spmbyma_period', $days, $base_url );
            ?>
                    ...
                    <?php else : ?>
                        <?php foreach ( $plugins as $p ) : ?>
                            ...
                            $status_class = match( $p['verification'] ) {
                                SPMBYMA_License_Detector::STATUS_VERIFIED     => 'success',
                                SPMBYMA_License_Detector::STATUS_NEEDS_REVIEW => 'danger',
                                default                                   => 'warning',
                            };
                            ?>
